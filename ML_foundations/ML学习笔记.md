...
1. 感知机的模型为$f(x)=\mathtt{sign}(w^Tx+b)$. 

2. 对于感知机学习算法的对偶形式的理解。

   看了知乎上对于对偶形式动机推理，在初始形式的时候，每次迭代我们需要计算$w^Tx+b$，对于$n$维空间上的向量计算，若$n$很大，那么计算量也很大。但是引入对偶形式：首先看到：$w:=w+\eta x_iy_i, b:=b+\eta y_i$. 每次发生错误分类的样本，就对变量进行更新，那么就会得到：
   $$
   w=\sum_{i=1}^N \eta_i y_i x_i\ \ \ \ \ \ \ \ \ b=\sum_{i=1}^N\eta_i y_i
   $$

那么，就会得到对偶形式的感知机模型$f(x)=\mathtt{sign}(\sum_{i=1}^n \eta_i y_i x_i x + b)$. 观察这个式子，发现他只是对原先式子的改写，并且，因为每次计算内积部分的运算是不变的，所以可以事先计算好这些值，即Gram矩阵。
...
$$
即，它所表示的数据所在的空间并不是线性可分的。

2.2 略

2.3 先看凸壳的定义：

**定义：** 设集合$S\subset R^n$，是由$R^n$中的$k$个点所组成的集合，即$S=\{x_1,x_2,\cdots x_k\}$。定义$S$的凸壳$\mathtt{conv}(S)$为：
$$
\mathtt{conv}(S)=\{x=\sum_{i=1}^k \lambda_ix_i|\sum_{i=1}^k\lambda_i=1, \lambda_i\ge0 \}
$$
**Proof:**

充分性：

样本集合线性可分，即存在超平面将样本的正反例完全分开。根据凸壳的定义，凸是所有样本点的线性组合，最后的所得就是样本的边缘。正反例的边缘应当是有严格的分界。

必要性：

若凸壳相交，那么就不存在超平面可以将样本集合完全的分开。